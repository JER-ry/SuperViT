...
class RandomResizedCropAndInterpolationWithCoords(RandomResizedCropAndInterpolation):
    ...
    def __call__(self, img, label_map):
        ...
        coords = (i / img.size[1], j / img.size[0], h / img.size[1], w / img.size[0])
        ...
        return (
            torchvision_F.resized_crop(img, i, j, h, w, self.size, interpolation),
            label_map,
        )
...
def transforms_imagenet_train(
    ...
    final_tfl = [
        RandomResizedCropAndInterpolationWithCoords(
            size=img_size, scale=scale, ratio=ratio, interpolation=interpolation
        )
    ]
