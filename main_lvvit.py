...
# import tlt.models
import lvvit.models.super_lvvit
from lvvit.data import (
...
from lvvit.loss import TokenLabelCrossEntropy, TokenLabelSoftTargetCrossEntropy
from lvvit.utils import load_pretrained_weights
...
parser.add_argument(
    "--autoresume", action="store_true", help="auto resume from checkpoint"
)
parser.add_argument(
    "--no-resume-opt",
    action="store_true",
...
def main():
    ...
    if use_amp == "native":
        ...
        loss_scaler = NativeScaler_No_Step()
        if args.local_rank == 0:
    ...
    resume_epoch = None
    if args.autoresume and os.path.exists(os.path.join(args.output, "last.pth.tar")):
        args.resume = os.path.join(args.output, "last.pth.tar")
    if args.resume:
    ...
    train_loss_fn = TokenLabelCrossEntropy(
    ...
    if args.local_rank == 0:
        ...
        output_dir = get_outdir(output_base)
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            # eval_metrics = super_evaluate(loader_eval,model, device)
...
def get_training_sub_network(
    ...
    return sub_network, img_size_list
...
def train_one_epoch(
    ...
    ce_layer = SoftTargetCrossEntropy()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        sub_networks, img_size_list = get_training_sub_network()
        lam = np.random.beta(1, 1)
        bbx1, bby1, bbx2, bby2 = rand_bbox(14, lam)
        bbox = []
        for i in [8, 10, 12, 14]:
            scale = 14 / i
            bbox.append(
        ...
        if not args.prefetcher:
            ...
            if mixup_fn is not None:
                ...
                if len(target.shape) == 1:
                    target = create_token_label_target(
                        target, num_classes=args.num_classes, smoothing=args.smoothing
                    )
        else:
        ...
        for index, sub_network in enumerate(sub_networks):
            with amp_autocast():
                bbx = bbox[img_size_list.index(sub_network[0])]
                output = model(input, sub_network[0], sub_network[1], bbx)

                if index == 0:
                    loss, new_target = loss_fn(output, target)
                    max_subnetwork_loss = loss
                    max_outputs = output[0]
                    max_output_detach_softmax = F.softmax(max_outputs.detach(), dim=1)
                elif index == 2:
                    loss = ce_layer(output, new_target)
                    max_outputs = output
                else:
                    log_softmax_result = F.log_softmax(output, dim=1)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                reduced_loss = reduce_tensor(max_subnetwork_loss.data, args.world_size)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    ...
    with torch.no_grad():
        ...
        if args.local_rank == 0:
            ...
            _logger.info("{0}".format(log_name))
            for sub_network in sub_networks:
                _logger.info(
                    "* {image_size}_{keep_ratio}_Acc@1 {top1.global_avg:.3f} {image_size}_{keep_ratio}_Acc@5 {top5.global_avg:.3f} ".format(
                        ...
                        top1=metric_logger.meters[
                            f"{sub_network[0]}_{sub_network[1]}_acc1"
                        ],
                        top5=metric_logger.meters[
                            f"{sub_network[0]}_{sub_network[1]}_acc5"
                        ],
                    )
                )
