...
def create_stem(
    in_chs, out_chs, stem_type="", preact=True, conv_layer=None, norm_layer=None
):
    stem = OrderedDict()
...
class ResNetV2(nn.Module):
    ...
    def __init__(
        ...
        stem_feat = (
            ("stem.conv3" if "deep" in stem_type else "stem.conv")
            if preact
            else "stem.norm"
        )
    ...
    def forward(self, x):
        ...
        if not self.head.global_pool.is_identity():
            x = x.flatten(
                1
            )  # conv classifier, flatten if pooling isn't pass-through (disabled)
        return x
            ...
            for i, (sname, stage) in enumerate(self.stages.named_children()):
                for j, (bname, block) in enumerate(stage.blocks.named_children()):
                    ...
                    block.conv1.weight.copy_(
                        tf2th(weights[f"{block_prefix}a/{convname}/kernel"])
                    )
                    block.conv2.weight.copy_(
                        tf2th(weights[f"{block_prefix}b/{convname}/kernel"])
                    )
                    block.conv3.weight.copy_(
                        tf2th(weights[f"{block_prefix}c/{convname}/kernel"])
                    )
                    block.norm1.weight.copy_(
                        tf2th(weights[f"{block_prefix}a/group_norm/gamma"])
                    )
                    block.norm2.weight.copy_(
                        tf2th(weights[f"{block_prefix}b/group_norm/gamma"])
                    )
                    block.norm3.weight.copy_(
                        tf2th(weights[f"{block_prefix}c/group_norm/gamma"])
                    )
                    block.norm1.bias.copy_(
                        tf2th(weights[f"{block_prefix}a/group_norm/beta"])
                    )
                    block.norm2.bias.copy_(
                        tf2th(weights[f"{block_prefix}b/group_norm/beta"])
                    )
                    block.norm3.bias.copy_(
                        tf2th(weights[f"{block_prefix}c/group_norm/beta"])
                    )
