...
def _rep_vgg_bcfg(d=(4, 6, 16, 1), wf=(1.0, 1.0, 1.0, 1.0), groups=0):
    ...
    bcfg = tuple(
        [
            BlocksCfg(type="rep", d=d, c=c * wf, gs=group_size)
...
model_cfgs = dict(
    gernet_l=ByobCfg(
        blocks=(
            BlocksCfg(type="basic", d=1, c=128, s=2, gs=0, br=1.0),
            BlocksCfg(type="basic", d=2, c=192, s=2, gs=0, br=1.0),
            BlocksCfg(type="bottle", d=6, c=640, s=2, gs=0, br=1 / 4),
            BlocksCfg(type="bottle", d=5, c=640, s=2, gs=1, br=3.0),
            BlocksCfg(type="bottle", d=4, c=640, s=1, gs=1, br=3.0),
        ),
        stem_chs=32,
        num_features=2560,
    ),
    gernet_m=ByobCfg(
        blocks=(
            ...
            BlocksCfg(type="basic", d=2, c=192, s=2, gs=0, br=1.0),
            BlocksCfg(type="bottle", d=6, c=640, s=2, gs=0, br=1 / 4),
            BlocksCfg(type="bottle", d=4, c=640, s=2, gs=1, br=3.0),
            BlocksCfg(type="bottle", d=1, c=640, s=1, gs=1, br=3.0),
        ),
        stem_chs=32,
        num_features=2560,
    ),
    gernet_s=ByobCfg(
        blocks=(
            BlocksCfg(type="basic", d=1, c=48, s=2, gs=0, br=1.0),
            BlocksCfg(type="basic", d=3, c=48, s=2, gs=0, br=1.0),
            BlocksCfg(type="bottle", d=7, c=384, s=2, gs=0, br=1 / 4),
            BlocksCfg(type="bottle", d=2, c=560, s=2, gs=1, br=3.0),
            BlocksCfg(type="bottle", d=1, c=256, s=1, gs=1, br=3.0),
        ),
        stem_chs=13,
        num_features=1920,
    ),
...
def create_downsample(type, **kwargs):
    if type == "avg":
        ...
        return ConvBnAct(
            kwargs.pop("in_chs"), kwargs.pop("out_chs"), kernel_size=1, **kwargs
        )
...
class RepVggBlock(nn.Module):
    ...
    def __init__(
        ...
        layer_cfg = layer_cfg or {}
        act_layer, norm_layer, attn_layer = _ex_tuple(
            layer_cfg, "act_layer", "norm_layer", "attn_layer"
        )
    ...
    def __init__(
        ...
        for stage_idx, block_cfg in enumerate(cfg.blocks):
            ...
            if stride != 1:
                self.feature_info.append(
                    dict(num_chs=prev_chs, reduction=net_stride, module=prev_name)
                )
        ...
        if cfg.num_features:
            ...
            self.final_conv = ConvBnAct(
                prev_chs, self.num_features, 1, **_na_args(layer_cfg)
            )
        else:
            self.num_features = prev_chs
            self.final_conv = nn.Identity()
        self.feature_info += [
            dict(num_chs=self.num_features, reduction=net_stride, module="final_conv")
        ]
