...
def _nfnet_cfg(
    ...
    attn_kwargs = (
        attn_kwargs if attn_kwargs is not None else dict(reduction_ratio=0.5, divisor=8)
    )
    cfg = NfCfg(
        depths=depths,
        channels=channels,
...
model_cfgs = dict(
    ...
    nfnet_l0a=_nfnet_cfg(
        depths=(1, 2, 6, 3),
        channels=(256, 512, 1280, 1536),
        feat_mult=1.5,
        group_size=64,
        bottle_ratio=0.25,
        attn_kwargs=dict(reduction_ratio=0.25, divisor=8),
        act_layer="silu",
    ),
    ...
    nf_seresnet26=_nfres_cfg(
        depths=(2, 2, 2, 2), attn_layer="se", attn_kwargs=dict(reduction_ratio=1 / 16)
    ),
    nf_seresnet50=_nfres_cfg(
        depths=(3, 4, 6, 3), attn_layer="se", attn_kwargs=dict(reduction_ratio=1 / 16)
    ),
    nf_seresnet101=_nfres_cfg(
        depths=(3, 4, 23, 3), attn_layer="se", attn_kwargs=dict(reduction_ratio=1 / 16)
    ),
...
class NormFreeNet(nn.Module):
    ...
    def __init__(
        ...
        if cfg.gamma_in_act:
            ...
            conv_layer = partial(
                conv_layer, eps=1e-4
            )  # DM weights better with higher eps
        else:
