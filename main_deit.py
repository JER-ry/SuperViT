...
from deit.engine import super_train_one_epoch, super_evaluate
from deit.losses import DistillationLoss
from samplers import RASampler
from utils import NativeScaler_No_Step
import deit.super_deit
import utils
import shutil
import pdb
...
def main(args):
    ...
    if args.eval:
        test_stats = super_evaluate(data_loader_val, model, device)
        print(
            f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['224_1_acc1']:.1f}%"
        )
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = super_train_one_epoch(
            model,
            criterion,
            data_loader_train,
            optimizer,
        ...
        if utils.is_main_process() and max_accuracy < test_stats["224_1_acc1"]:
            shutil.copyfile(checkpoint_path, f"{args.output_dir}/model_best.pth")
        max_accuracy = max(max_accuracy, test_stats["224_1_acc1"])
